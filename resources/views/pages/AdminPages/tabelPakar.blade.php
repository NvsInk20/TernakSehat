...
<body class="bg-gray-100 min-h-full flex">
    ...
    <div class="max-w-[60rem] mx-auto mb-[18%]" id="tabelGejala">
        ...
        <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3 bg-customOrange">
            <div>
                ...
                <p class="text-white font-bold">Data Ahli Pakar</p>
            </div>
            <div class="ml-3">
                <div class="w-full max-w-sm min-w-[200px] relative">
                    <form method="GET" action="{{ route('admin.pakar') }}" class="relative mr-3">
                        <input
                            class="bg-white w-full pr-11 h-10 pl-3 py-2 bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded transition duration-200 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md"
                            placeholder="Cari Pakar..." name="search" value="{{ request('search') }}" />
                        <button
            ...
            <table class="w-full table-fixed text-sm text-gray-700">
                <thead class="bg-gray-50 text-gray-600 text-center">
                    <tr>
                        ...
                        <th class="border border-gray-300 px-4 py-2">Kode Pakar</th>
                ...
                <tbody>
                    @forelse ($pakars as $index => $user)
                        <tr class="hover:bg-gray-50">
                            <td class="border border-gray-300 px-4 py-2 w-16 text-center">{{ $index + 1 }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $user->kode_ahliPakar }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $user->nama }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">
                                @if ($user->dokumen_pendukung)
                                    <a href="{{ route('viewDokumen', ['fileName' => basename($user->dokumen_pendukung)]) }}"
                                        target="_blank">
                                        <button type="button"
                                            class="px-4 py-2 bg-blue-500 text-white font-bold rounded-md hover:bg-blue-600">
                                            Cetak PDF
                                        </button>
                                    </a>
                                @else
                                    <span class="text-gray-500">Dokumen Tidak Tersedia</span>
                                @endif
                            ...
                            <td class="border border-gray-300 px-4 py-2 text-center">
                                @if ($user->status == 'active')
                                    <form action="{{ route('admin.toggleStatus', $user->kode_ahliPakar) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit"
                                ...
                                @else
                                    <form action="{{ route('admin.toggleStatus', $user->kode_ahliPakar) }}"
                                        ...
                                        @method('PUT')
                                        <button type="submit"
                                            class="px-4 py-2 bg-red-500 text-white font-bold rounded-md hover:bg-red-600">
                                            Belum Akses
                                        </button>
                                    </form>
                                @endif
                            ...
                            <td class="border border-gray-300 px-4 py-2 flex justify-center ">
                                <a href="{{ route('admin.editPakar', ['role' => $user->role, 'kode' => $user->kode_ahliPakar]) }}"
                                ...
                                <!-- Form Delete -->
                                <form id="deleteForm-{{ $user->kode_ahliPakar }}"
                                    action="{{ route('admin.deletePakar', $user->kode_ahliPakar) }}" method="POST"
                                    style="display:none;">
                                    @csrf
                                    @method('DELETE')
                                </form>

                                <button onclick="confirmDelete('{{ $user->kode_ahliPakar }}')"
    ...
    @empty
        ...
        <div class="flex justify-between items-center px-6 py-4 bg-gray-50">
            <div class="text-sm text-gray-500">
                Menampilkan <b>{{ $pakars->firstItem() ?? 0 }}-{{ $pakars->lastItem() ?? 0 }}</b> dari
                {{ $pakars->total() }}
            </div>
            <div class="flex space-x-2 items-center">
                <p class="text-sm text-gray-500">Showing {{ $pakars->firstItem() ?? 0 }} to
                    {{ $pakars->lastItem() ?? 0 }} of {{ $pakars->total() }}</p>
                <div class="flex space-x-1">
                    @if ($pakars->onFirstPage())
                        <button
                    ...
                    @foreach ($pakars->getUrlRange(1, $pakars->lastPage()) as $page => $url)
                        @if ($page == $pakars->currentPage())
                            <button
                    ...
                    @if ($pakars->hasMorePages())
                        <a href="{{ $pakars->nextPageUrl() }}"
                            class="px-3 py-1 min-w-9 min-h-9 text-sm font-normal text-slate-500 bg-white border border-slate-200 rounded hover:bg-slate-50 hover:border-slate-400 transition duration-200 ease">
                            Next
                        </a>
                    @else
    ...
    <script>
        function confirmDelete(kode_ahliPakar) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById(`deleteForm-${kode_ahliPakar}`).submit();
                }
            });
        }
