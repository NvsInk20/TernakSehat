...
<head>
    ...
    <title>Edit Aturan Penyakit</title>
    <!-- Alpine.js -->
...
<body class="bg-orange-100 min-h-screen flex items-center justify-center">
    ...
    <div class="bg-white shadow-lg rounded-lg w-full xl:my-24 max-w-3xl mx-auto p-6 sm:p-10">
        ...
        <h1 class="text-3xl font-semibold text-center mb-6 text-gray-800">Edit Aturan Penyakit</h1>

        <form action="{{ route('aturanPenyakitPakar.update', $kode_relasi) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <select id="kode_penyakit" name="kode_penyakit" required
                    ...
                    @foreach ($penyakit as $p)
                        <option value="{{ $p->kode_penyakit }}"
                            {{ $aturanPenyakit->first()->kode_penyakit == $p->kode_penyakit ? 'selected' : '' }}>
            ...
            <div id="gejala-container" class="mb-6 space-y-4">
                ...
                @foreach ($aturanPenyakit as $aturan)
                    <div class="flex items-center space-x-4">
                        <select name="kode_gejala[]" required
                            ...
                            @foreach ($gejala as $g)
                                <option value="{{ $g->kode_gejala }}"
                                    {{ $aturan->kode_gejala == $g->kode_gejala ? 'selected' : '' }}>
                        ...
                        <select name="jenis_gejala[]" required
                            ...
                            <option value="wajib" {{ $aturan->jenis_gejala == 'wajib' ? 'selected' : '' }}>Wajib
                            </option>
                            <option value="opsional" {{ $aturan->jenis_gejala == 'opsional' ? 'selected' : '' }}>
                                Opsional</option>
                        </select>
                        <button type="button"
            ...
            <div class="mb-6">
                ...
                <select id="kode_rekomendasi" name="kode_rekomendasi" required
                    ...
                    @foreach ($rekomendasi as $s)
                        <option value="{{ $s->kode_rekomendasi }}"
                            {{ $aturanPenyakit->first()->kode_rekomendasi == $s->kode_rekomendasi ? 'selected' : '' }}>
                            {{ $s->kode_rekomendasi }} - {{ $s->rekomendasi }}
                        </option>
                    @endforeach
                </select>
            </div>
