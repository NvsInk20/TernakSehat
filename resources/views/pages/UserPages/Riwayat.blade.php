    ...
    <div class="max-w-6xl mx-auto mb-12 2xl:min-h-screen">
            ...
            <form method="GET" action="{{ route('riwayatDiagnosa.index') }}" class="mx-4">
                <div class="flex items-center space-x-2">
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                        ...
                        class="px-4 py-2 border 2xl:text-xl rounded-md text-black focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                    <button type="submit"
                        class="px-4 py-2 bg-blue-600 text-white 2xl:text-xl rounded-md hover:bg-blue-700 focus:outline-none">
                        Cari
                    </button>
                </div>
            </form>
        ...
        <div class="bg-white rounded-lg 2xl:w-[80rem] shadow-md overflow-hidden 2xl:text-xl">
            ...
            <div class="flex justify-between items-center px-6 py-4 bg-gray-50">
                <div class="text-sm text-gray-500 2xl:text-xl">
                    @php
                ...
                <div class="flex space-x-2 items-center">
                    ...
                    <div class="flex space-x-1">
                        ...
                        @if ($riwayatPaginated->onFirstPage())
                            <button
                                class="px-3 py-1 min-w-9  2xl:text-xl min-h-9 text-sm font-normal text-gray-400 bg-white border border-gray-200 rounded cursor-not-allowed">
                                Prev
                            </button>
                        @else
                            <a href="{{ $riwayatPaginated->previousPageUrl() }}"
                                class="px-3 py-1 min-w-9  2xl:text-xl min-h-9 text-sm font-normal text-slate-500 bg-white border border-slate-200 rounded hover:bg-slate-50 hover:border-slate-400 transition duration-200 ease">
                                Prev
                            </a>
                        @endif
                        ...
                        @foreach ($riwayatPaginated->getUrlRange(1, $riwayatPaginated->lastPage()) as $page => $url)
                            @if ($page == $riwayatPaginated->currentPage())
                                <button
                                    class="px-3 py-1 min-w-9  2xl:text-xl min-h-9 text-sm font-normal text-white bg-slate-800 border border-slate-800 rounded hover:bg-slate-600 hover:border-slate-600 transition duration-200 ease">
                                    {{ $page }}
                                </button>
                            @else
                                <a href="{{ $url }}"
