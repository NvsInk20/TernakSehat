    ...
    <form method="GET" x-data="{ selectedPage: '{{ request()->segment(2) }}', openDropdown: false }">
        <div>
            ...
            <button @click.prevent="openDropdown = !openDropdown" type="button" class="dropdown-btn">
                <span x-text="selectedPage === 'users' ? 'Data Pengguna' : 'Data Ahli Pakar'"></span>
        ...
        <!-- Item dropdown -->
        <div class="dropdown-items" x-show="openDropdown" @click.outside="openDropdown = false" x-transition>
            <a href="/admin/users"
                @click.prevent="selectedPage = 'users'; window.location.href = '/admin/users'; openDropdown = false"
                class="{{ request()->segment(2) === 'users' ? 'active' : '' }}">Data Pengguna</a>
            <a href="/admin/pakar"
                @click.prevent="selectedPage = 'pakar'; window.location.href = '/admin/pakar'; openDropdown = false"
                class="{{ request()->segment(2) === 'pakar' ? 'active' : '' }}">Data Ahli Pakar</a>
        </div>
    </form>
</div>
<style>
