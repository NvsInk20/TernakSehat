...
// Ahli Pakar
    ...
    public function storeByPakar(Request $request)
    {
        ...
        // Mengarahkan kembali ke halaman daftar penyakit dengan pesan sukses
        return redirect()->route('solusiPakar.create')->with('success', 'Solusi berhasil ditambahkan!');
    }
     ...
     * Menampilkan form untuk mengedit penyakit berdasarkan ID
     */
    public function editByPakar($kode_solusi)
    {
        $solusi = solusi::where('kode_solusi', $kode_solusi)->firstOrFail();
     ...
     * Memperbarui data penyakit berdasarkan kode_solusi.
     */
    public function updateByPakar(Request $request, $kode_solusi)
    {
        // Validasi input
        $request->validate([
            'solusi' => 'required|string|max:10000',
        ...
        // Redirect dengan pesan sukses
        return redirect()->route('solusiPakar.edit', ['kode_solusi' => $kode_solusi])
            ->with('success', 'Solusi berhasil diperbarui!');
    }
     ...
     * Menghapus penyakit berdasarkan ID
     */
    public function destroyByPakar($kode_solusi)
{
    $solusi = solusi::where('kode_solusi', $kode_solusi)->first();

    if ($solusi) {
        $solusi->delete();
        return redirect()->route('Pakar.solusi')->with('success', 'Solusi berhasil dihapus.');
    }

    return redirect()->route('Pakar.solusi')->with('error', 'Solusi tidak ditemukan.');
}
